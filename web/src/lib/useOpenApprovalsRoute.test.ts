...
describe('useOpenApprovalsRoute', () => {
  it('navigates to /access only when the signal counter INCREASES, not on re-render', () => {
    ...
    // on Access — every tab switch bounces back).
    ...
    expect(navigate2).toHaveBeenCalledWith('/access')
  })
})
