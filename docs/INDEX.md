...
- `decisions/` — ADRs (one architectural decision each, `NNNN-slug.md`):
  ...
  - [`0042-access-page-grant-first-class.md`](architecture/decisions/0042-access-page-grant-first-class.md) — merge the Agents/Operations/Approvals pages into one **Access** page (nav: Dashboard·Upstreams·Access·Audit·Settings); the grant (agent×upstream), derived from `listRules()`+`listAccessRequests()`, becomes the organizing object (pending queue on top, grants grouped by agent/upstream below); Revoke relocates onto the grant via new `POST /grants/revoke` (`DeleteBySubjectUpstream` + `MarkRevokedBySubjectUpstream`), superseding the old per-request revoke for the UI; agent delete/meta move onto the Access agent card; enum value-set editing dropped from the UI (still enforced); access-request history moves to a read-only Audit "Запросы прав" tab; `desktop.open-approvals` retargets to `/access`; notes the pre-existing host-access-only-grant gap (no rule ⇒ invisible in "Выданные права") made newly visible.
  - [`0043-getatts-read-workspace-relaxation.md`](architecture/decisions/0043-getatts-read-workspace-relaxation.md) — a Citeck get-atts read (`records:[ref]`, `scopeUnknown`) is authorized by a **concrete-workspace** read grant (source gate still applies); the workspace is not derivable from a ref and read is low-risk. Writes-by-ref (mutate/delete) stay conservative; `scopeAll` unchanged. `matchWorkspace` gains an `opKind` arg. Refines ADR-0039's "get-atts all-or-nothing" note.
  - [`0044-operator-edit-disclosure.md`](architecture/decisions/0044-operator-edit-disclosure.md) — the agent is told when the operator **narrows** its preset request: `access_requests.edits` (JSON `[]BindingEdit`) captured at grant time via `access.DiffBindings(requested, granted)`, surfaced on `mcpsvc.AccessResult.operator_edits` + a `memo` note (`operator narrowed workspace: * → ECOSENT`) by both `get-access` and `request-preset`. `GrantLatest` gains an `edits` param; migration `access_request_edits`.
...
- `specs/` — brainstormed design specs.
  ...
  - [`2026-07-07-access-page-redesign.md`](superpowers/specs/2026-07-07-access-page-redesign.md) — Access page redesign: merge Operations/Approvals/Agents into one grant-first-class **Access** page (pending queue + grouped grants), move access-request history to an Audit tab. ADR-0042.
- `plans/` — bite-sized implementation plans.
  ...
  - Access page redesign: [`2026-07-07-access-page-redesign.md`](superpowers/plans/2026-07-07-access-page-redesign.md) (grant-scoped revoke backend, Access page components, Audit history tab, nav/routes; ADR-0042). Spec: [`2026-07-07-access-page-redesign.md`](superpowers/specs/2026-07-07-access-page-redesign.md).
