...
**Fan-out atomicity** (resolved by ADR-0038). Both `approvePreset` and `approveK8sAccess` call
`policy.Registry.CreateMany`, which writes all rules in a single SQL transaction (all-or-nothing): a
mid-batch failure rolls back the entire batch, so no partial grant can be left in place. See
[ADR-0038](0038-atomic-rule-fanout.md).
