...
- **Data model.** `policy.Rule` gains `ExpiresAt time.Time` (zero value = never expires).
  ...
  expiry).
- **Enforcement lives in `Decide` only, on both the raw-HTTP and server-profile paths.** After
  `ForUpstream` loads the upstream's rules, `Decide` filters out any rule with
  `!ExpiresAt.IsZero() && ExpiresAt.Before(now)` (`now = time.Now().UTC()`) *before* either
  matching path runs, so an expired rule cannot allow, deny, or require-approval — it is simply
  absent, and default-deny applies. `ForUpstream` and `List` (the admin rule-list) stay
  **unfiltered**: the UI needs to see expired rules in order to mark and renew them.
...
- **Renewal.** A new operator-gated `POST /rules/{id}/renew` (`hRuleRenew`) takes `{ttl_seconds}`
  ...
  permanent). `GET /rules` exposes `expires_at` so the UI can render it.
...
- The operation approval's existing "extend an existing rule with a new allowed value" path now
  also refreshes that rule's `expires_at` on every re-approval — an agent whose value set keeps
  growing via repeated approvals also keeps its grant alive, which is the intended behavior (the
  operator is actively re-affirming access each time they approve).
...
- Covered by tests: `internal/policy` (expired rules excluded from `Decide` on both matching
  ...
  `insertRule`/`scanRows`/`CreateMany` carry `ExpiresAt`), `internal/store` (`rule_expiry`
  migration upgrades an old DB; a fresh DB has the column), `internal/daemon` (approve
  preset/operation/k8s with `ttl_seconds` stamps `expires_at`; manual create; `POST
  /rules/{id}/renew`; `ttl_seconds:0` ⇒ permanent), and the web layer (`DurationSelect` options/
  default/never, `RuleRow` chip/badge/renew, grant-card aggregate badge, `api.ts` payload shapes).
